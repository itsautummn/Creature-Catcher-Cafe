...
func process_physics(delta : float) -> TestCreatureState:
	...
	if move_prob == 100:
		...
		## New
		# Chose a random position to move to
		var random_pos = Vector3(randf_range(-100.0, 100.0), 0.0, randf_range(-100.0, 100.0))
		# Normalize the position to get the direction
		var random_dir = random_pos.normalized()
		# Choose how long to move for, between 1 and 5 seconds
		var random_time = randf_range(1.0, 5.0)
		# Set the move direction and time
		move_state.move_dir = random_dir
		move_state.move_time = random_time
